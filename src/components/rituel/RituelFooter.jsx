...
export default function RituelFooter() {
  const year = new Date().getFullYear()
  return (
    <footer style={{ background: '#120610', borderTop: `1px solid ${PINK}18` }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div>
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 rounded-full overflow-hidden border flex-shrink-0"
                ...
                <img src="/images/rituel/logo.jpg" alt="Rituel de Beauté"
                  className="w-full h-full object-cover"
                  onError={(e) => {
                    e.target.parentElement.innerHTML = `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-family:'Playfair Display',serif;color:${PINK};font-weight:700;font-size:16px">RB</div>`
                  }}
                />
              </div>
              <div>
                <p className="font-heading font-bold text-white text-base leading-none"
                  style={{ fontFamily: "'Playfair Display', serif" }}>Rituel de Beauté</p>
                <p className="text-xs tracking-widest uppercase mt-0.5" style={{ color: PINK }}>by APISAHI Group</p>
              </div>
            </div>
            <p className="text-sm leading-relaxed mb-6" style={{ color: 'rgba(255,255,255,0.4)' }}>
              Une approche holistique du soin alliant beauté naturelle, bien-être intérieur et rituels personnalisés pour révéler votre éclat naturel.
            </p>
            <div className="flex items-center gap-3">
              {SOCIALS.map((s) => (
                <a key={s.name} href={s.href} target="_blank" rel="noreferrer" aria-label={s.name}
                  className="w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200"
                  style={{ border: `1px solid ${PINK}25`, color: 'rgba(255,255,255,0.4)' }}
                  onMouseEnter={(e) => { e.currentTarget.style.borderColor = PINK; e.currentTarget.style.color = PINK_LIGHT }}
                  onMouseLeave={(e) => { e.currentTarget.style.borderColor = `${PINK}25`; e.currentTarget.style.color = 'rgba(255,255,255,0.4)' }}>
                  {s.icon}
                </a>
              ))}
            </div>
          ...
          <div>
            <h3 className="font-heading font-semibold text-white text-base mb-6 relative inline-block"
              ...
              Navigation
              <span className="absolute -bottom-1 left-0 w-6 h-px" style={{ background: PINK }} />
            </h3>
          ...
          <div>
            ...
            <Link to="/"
              ...
              onMouseLeave={(e) => e.currentTarget.style.color = `${PINK}50`}>
      ...
      <div style={{ borderTop: `1px solid ${PINK}10` }}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-xs" style={{ color: 'rgba(255,255,255,0.2)' }}>
            © {year} Rituel de Beauté — APISAHI Group. Tous droits réservés.
          </p>
          <p className="text-xs" style={{ color: `${PINK}40` }}>Révélez votre éclat naturel</p>
        </div>
      </div>
    </footer>
  )
